...
affixes as (
    select
        line,
        substring(line, 1, n) as prefix,
        substring(line, length(line) - n + 1, n) as suffix
    from
        lines cross join generate_series(1, length(line)) as n
),
first as (
    select
        line,
    ...
    from
        affixes
    where
        regexp_match(prefix, '([0-9]|zero|one|two|three|four|five|six|seven|eight|nine)$') is not null
    group by line
),
first_digit as (
    select
...
min_suffix as (
    select
        line,
        min(length(suffix)) as length
    from
        affixes
    where
        regexp_match(suffix, '^([0-9]|zero|one|two|three|four|five|six|seven|eight|nine)') is not null
    group by line
),
last as (
    select
        ...
        suffix
    from
        affixes join min_suffix on (
            affixes.line = min_suffix.line and 
            length(affixes.suffix) = min_suffix.length
        )
),
last_digit as (
    select
...
calibration_values as (
    select
        (first_digit.digit || last_digit.digit)::integer as value
    from
        first_digit join last_digit on first_digit.line = last_digit.line
)
select
    sum(value) as answer
from
