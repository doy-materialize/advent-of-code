with
input as (
    select 'Game 1: 7 red, 8 blue; 6 blue, 6 red, 2 green; 2 red, 6 green, 8 blue; 9 green, 2 red, 4 blue; 6 blue, 4 green
...
Game 100: 10 red; 11 blue, 12 red; 1 green, 7 blue, 6 red' as input
),
lines as (
    select
...
games as (
    select
        regexp_match(line, 'Game (\d+): (.*)') as match
    from
        lines
),
draws as (
    select
        match[1]::int as game_id,
        row_number() over (partition by match[1]) as draw_id,
        regexp_split_to_table(match[2], '; ') as draw
    from
        games
),
color_strs as (
    select
        game_id,
        draw_id,
        regexp_split_to_table(draw, ', ') as color
    from
        draws
),
colors as (
    select
        game_id,
        draw_id,
        sum(regexp_match(color, '(\d+) red')[1]::int) as red,
        sum(regexp_match(color, '(\d+) green')[1]::int) as green,
        sum(regexp_match(color, '(\d+) blue')[1]::int) as blue
    from
        color_strs
...
select
    sum(coalesce(red, 0) * coalesce(green, 0) * coalesce(blue, 0)) as answer
from
    max_colors
