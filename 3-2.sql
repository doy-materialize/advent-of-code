with
input as (
    select '.........426.............985.........40..........207............................841..463................................633........17.384...
...
...............726.....308.............%........*...../.+........=..../146.................*...................509..........*........593....
930.........................823..............994.................................100.....857.......................708.220.184..............' as input
),
rows_array as (
    select
...
rows as (
    select
        row_num,
        rows_array.row[row_num] as row
    from
        rows_array,
        generate_subscripts(rows_array.row, 1) as row_num
    order by row_num
),
cols as (
    select
        row_num,
        col_num,
        substring(rows.row, col_num, 1) as cell
    from
        rows,
        generate_series(1, length(rows.row)) as col_num
...
group_arrays as (
    select
        row_num,
        regexp_split_to_array(rows.row, '\b') as groups
    from
        rows
),
groups as (
    select
        row_num,
        group_num,
        groups[group_num] as group
    from
        group_arrays,
        generate_subscripts(groups, 1) as group_num
...
ends as (
    select
        row_num,
        group_num,
        sum(length(groups.group)) over (partition by row_num order by group_num rows between unbounded preceding and current row) as pos
    from
        groups
...
positions as (
    select
        row_num,
        group_num,
        1 + lag(pos, 1, 0) over (partition by row_num order by group_num) as start,
        pos as end
    from
        ends
),
number_ranges as (
    select
        groups.row_num,
        groups.group_num,
        groups.group::int as number,
        start,
        end
    from
        groups join positions on (
            groups.row_num = positions.row_num and
            groups.group_num = positions.group_num
        )
    where
        regexp_match(groups.group, '^\d+$') is not null
),
numbers as (
    select
        row_num,
        group_num,
        generate_series(start, end) as col_num,
        number
    from
        number_ranges
),
gears_adjacent as (
    select
        gears.row_num as gear_row,
        gears.col_num as gear_col,
        row_num.row_num,
        col_num.col_num
    from
        gears,
        generate_series(row_num - 1, row_num + 1) as row_num,
        generate_series(col_num - 1, col_num + 1) as col_num
),
numbers_near_gears as (
    select
        numbers.row_num,
        numbers.group_num,
        max(numbers.col_num) as col_num,
        max(numbers.number) as number,
        max(gears_adjacent.gear_row) as gear_row,
        max(gears_adjacent.gear_col) as gear_col
    from
        numbers join gears_adjacent on (
            gears_adjacent.row_num = numbers.row_num and
            gears_adjacent.col_num = numbers.col_num
        )
    group by 1, 2
),
two_adjacent_gears as (
    select
        gear_row,
        gear_col,
        round(exp(sum(ln(number)))) as gear_ratio
    from
        numbers_near_gears
    group by 1, 2
    having
