...
numbers_near_parts as (
    select
        numbers.row_num,
        numbers.group_num,
        max(numbers.number) as number
    from
        numbers inner join parts_adjacent on (
            numbers.row_num = parts_adjacent.row_num and
            numbers.col_num = parts_adjacent.col_num
        )
    group by 1, 2
)
select
    sum(number) as answer
from
    numbers_near_parts
